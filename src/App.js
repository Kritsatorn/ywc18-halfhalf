  ...
  const Tomap = () => {
    return (
      <div>
              ...
              <form>
                <label style={{ fontWeight: "normal" }}>
                  <input
                    type="radio"
                    value={cat.name}
                    checked={value === cat.name}
                    onChange={(e) => {
                      setValue(e.target.value);
                      setCategories2(cat.subcategories);
                    }}
                  />
        ...
        <h4>
          <strong>ประเภท{value}</strong>
        </h4>

        <input
          type="radio"
          value="ทั้งหมด"
          checked={value2 === "ทั้งหมด"}
    ...
    if (value == "") {
              ...
              <div className="main col-md-3 col-sm-12 col-xs-12">
                <div className="cajaContenido col-sm-8">
                  ...
                  <div className="ShopDetail">
                    <h2>
                      <strong>{shop.shopNameTH}</strong>
                    </h2>
                    {shop.subcategoryName} | $ | {shop.addressDistrictName}{" "}
                    {shop.addressProvinceName} <br />
      ...
      return (
        <div>
          <div>
            {merchant
              .filter((shops) => shops.subcategoryName === value2)
              .map((shop2) => (
                <div className="main">
                  <div className="cajaContenido col-sm-12">
                    ...
                    <div className="ShopDetail">
                      <h2>
                        <strong>{shop2.shopNameTH}</strong>
                      </h2>
                      {shop2.subcategoryName} | $ | {shop2.addressDistrictName}{" "}
                      {shop2.addressProvinceName} <br />
  ...
  return (
    <div>
      <div className="navbar">
        <img className="LogoPic visible-lg visible-md visible-sm" src={logo} />
        <t />
        <s />
        <img className="LogoPic visible-xs" src={logo2} />
        <t />
        <s />
        <Dropdown
          className="drowdown visible-lg visible-md visible-sm"
          search
          selection
          trigger={trigger}
        />
        <Input
          ...
          fluid
          action={{ icon: "search" }}
          placeholder="ค้น ชื่อ ร้านอาหาร และ เครื่องดื่ม ร้านธงฟ้า ร้านOTOP และ ร้านทั่วไป"
        />
        <img className="visible-xs" src={filter} onClick={handleChange} />
        <t />
        <s />
      </div>
      ...
      <h3 style={{ paddingLeft: 10 }}>
        <strong>ผลการค้นหา {value} ทั้งหมด</strong>
      </h3>
      <br />
      <div className="menu">
        <Tomap />
